...
class JenkinsConnector:
    def __init__(self, jenkins_url=None, username=None, api_token=None, verify_ssl=None, timeout=30):
        ...
        if jenkins_url and '@' in jenkins_url:
            ...
            self.jenkins_url = jenkins_url.rstrip('/') if jenkins_url else os.environ.get('JENKINS_URL')
        ...
        # Connection settings - handle verify_ssl from environment if not explicitly set
        if verify_ssl is None:
            verify_ssl_env = os.environ.get('JENKINS_VERIFY_SSL', 'False')
            self.verify_ssl = verify_ssl_env.lower() in ('true', 'yes', '1', 't')
        else:
    ...
    def refresh_connection(self):
        """
        Refresh the connection to Jenkins server.
        This helps prevent stale data in long-running applications.

        :return: True if refresh was successful
        """
        try:
            logger.info("Refreshing Jenkins connection...")

            # Close existing session to free resources
            if self.session:
                self.session.close()

            # Create a new session with fresh connection
            self.session = requests.Session()
            self.session.auth = self.basic_auth
            self.session.headers.update(self.headers)
            self.session.verify = self.verify_ssl
    ...
    def get_latest_builds(self, limit=20):
        ...
        try:
            ...
            for job_name in all_jobs:
                try:
                    ...
                    if job_info and 'lastBuild' in job_info and job_info['lastBuild']:
                        ...
                        if last_build_number:
                            try:
                                # Get detailed build info with a shorter timeout
                                ...
                                logger.warning(f"Error getting build details for {job_name} #{last_build_number}: {e}")
                ...
                # Check if we have enough builds already to avoid long processing times
                if len(all_builds) >= limit * 2:  # Get more than needed to allow for sorting
                    break
    ...
    def get_build_info(self, job_name, build_number):
        ...
        try:
            ...
            # Use a shorter timeout for build info requests
            shorter_timeout = min(10, self.timeout)
            return self.get_jenkins_info(endpoint, timeout=shorter_timeout)
        except Exception as e:
